...
void vTask1(void *pvParameters) {
    for (;;) {
        printf("Working\n");
        vTaskDelay(pdMS_TO_TICKS(1000)); // Delay for 1 second (period)
    }
}
...
void vTask2(void *pvParameters) {
    ...
    for (;;) {
        celsius = (fahrenheit - 32) * 5 / 9;
        printf("Fahrenheit: %.2f, Celsius: %.2f\n", fahrenheit, celsius);
        vTaskDelay(pdMS_TO_TICKS(2000)); // Delay for 2 seconds (period)
    }
}
...
void vTask3(void *pvParameters) {
    ...
    for (;;) {
        result = num1 * num2;
        printf("Result of multiplication: %ld\n", result);
        vTaskDelay(pdMS_TO_TICKS(3000)); // Delay for 3 seconds (period)
    }
}
...
void vTask4(void *pvParameters) {
    ...
    for (;;) {
        ...
        vTaskDelay(pdMS_TO_TICKS(4000)); // Delay for 4 seconds (period)
    }
}
